const { TextEncoder, TextDecoder } = Components.utils.import("resource://gre/modules/Services.jsm", {});
...
export function createIndex(input) {
    try {
        ...
        wasm.createIndex(retptr, addHeapObject(input));
...
* @returns {any}
*/
export function runSearch(index, query, k) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        wasm.runSearch(retptr, addHeapObject(index), addHeapObject(query), k);
        var r0 = getInt32Memory0()[retptr / 4 + 0];
        var r1 = getInt32Memory0()[retptr / 4 + 1];
        var r2 = getInt32Memory0()[retptr / 4 + 2];
        if (r2) {
...
async function init(input) {
    if (typeof input === 'undefined') {
        input = new URL('vector_search_bg.wasm', import.meta.url);
    }
    const imports = getImports();

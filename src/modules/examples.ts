...
export class UIExampleFactory {
  ...
  static async registerReaderTabPanel() {
    const tabId = await ztoolkit.ReaderTabPanel.register(
      ...
      (panel: XUL.TabPanel | undefined, deck: XUL.Deck, win: Window, reader: _ZoteroTypes.ReaderInstance) => {
        if (!panel) {
          ztoolkit.log('This reader do not have right-side bar. Adding reader tab skipped.')
          return
        }
        ztoolkit.log(reader)
        const elem = ztoolkit.UI.createElement(win.document, 'vbox', {
